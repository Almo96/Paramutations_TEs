...
df_summary_diff <- df0 %>% 
  dplyr::group_by(rep, sampleid) %>%
  dplyr::summarize(max_shot = max(avtes), 
                   min_shot = min(avtes),
                   diff_shot= max_shot - min_shot,
                   av_tes = mean(avtes))


shotgun_phase <- ggplot(df_summary_diff, aes(x=as.factor(sampleid), y=diff_shot/av_tes)) + 
  geom_boxplot() +
  xlab("") +
  ylab("Shotgun phase avtes (max - min)/av_tes")
